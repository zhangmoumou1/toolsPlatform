...
from app.excpetions.DbException import DBError
...
class Mapper(object):
    ...
    @connect(True)
    async def insert(cls, *, model: QmsBase, session: AsyncSession = None, log=False, not_begin=False):
        session.add(model)
        await session.flush()
    ...
    @classmethod
    async def get_field_alias(cls, session, relation: Tuple[QmsRelationField], name, now, old=None):
        """
